...
router.get('/medias/:fkfilme', function(req, res, next) {
	console.log('Recuperando caminhões');
	
	//var idcaminhao = req.body.idcaminhao; // depois de .body, use o nome (name) do campo em seu formulário de login
	var idcaminhao = req.params.idcaminhao;
	var fkfilme = req.params.fkfilme;
	
	let instrucaoSql = "";
	
	if (env == 'dev') {
		// abaixo, escreva o select de dados para o Workbench
		instrucaoSql = `select avg(nota) from usuarios_analises where fk_analise=${fkfilme};`;
	} else if (env == 'production') {
		// abaixo, escreva o select de dados para o SQL Server
		instrucaoSql = `select avg(nota) from usuarios_analises where fk_analise=${fkfilme};`;
	} else {
	...
	sequelize.query(instrucaoSql, { type: sequelize.QueryTypes.SELECT })
	.then(resultado => {
		res.json(resultado[0]);  // joga pro resultado
	}).catch(erro => {
		console.error(erro);
		res.status(500).send(erro.message);
	});
